...
|---|---|---|
| **Voice-only mentions undetectable.** A user who records a voice memo saying "Hey Hermes" without typing `@` or using the tagging UI produces a transcript with no `@[name](guid)` markup. The mention is invisible to the gate. | Voice users in group channels can't reach the bot without typing. | Resolved when cv-api exposes `tagged_user_ids` on the message DTO. The forward-compat path in `is_user_mentioned()` already prefers the structured field; the moment it's populated, voice-tagged messages are detected. PR pending. |
...
| **`tagged_user_ids` not in API response.** The DB field is not surfaced in any DTO. | Worked around by parsing inline `@[name](guid)`. | PR open against cv-api. Once merged, no plugin changes needed. |
...
2. **`get_message()` caching.** Currently every reply fetches the parent on demand. A small LRU cache (~128 entries) cuts cost for star-shaped threads where many replies share a root.
